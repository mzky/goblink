...
func onException(param uintptr) uintptr {
	if miniDumpWriteDump == nil {
		log.Println("onException", "miniDumpWriteDump nil")
		return 0
	}
	pHandle := windows.CurrentProcess()
	dir := filepath.Join(TempPath, DllName)

	name := filepath.Join(dir, DllName+strconv.FormatInt(time.Now().Unix(), 32)+".dmp")
	fromString, err := syscall.UTF16PtrFromString(name)
	if err != nil {
		log.Println("onException.UTF16PtrFromString:"+name, err.Error())
		return 0
	}
	var sa windows.SecurityAttributes
	fHandle, err := windows.CreateFile(fromString, GENERIC_WRITE, FILE_SHARE_WRITE, &sa, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0)
	if err != nil {
		log.Println("onException.CreateFile:"+name, err.Error())
		return 0
	}
	...
	if success != 1 {
		log.Println("onException.miniDumpWriteDump", err.Error())
		return 0
	}
	return 0
}
